...
**Placement.** LeseDeutsch is a personal product project (`group: 'product'`, `kind: 'personal'`) with `order: 2`, so it follows SaveFold on the homepage and in the Projects list, as the owner requested. The later projects were renumbered by one; Kırıkkale Taksicin (defined in `src/content/projects.js`) moved from 6 to 7.

**Status and test link.** The status is `inDevelopment` ("In Entwicklung · Technische Beta"). The public test version (`https://staging.lesedeutsch.de/`) is stored in a new optional `previewUrl` field instead of `liveUrl`, so that an unreleased product is not labelled "Live-Website öffnen". Templates show a "Testversion öffnen / Open test version / Test sürümünü aç" link (`common.previewSite`) when a project has a `previewUrl` and no `liveUrl`: on the card, as the hero button and in the status box on the detail page. The media frame shows the preview domain. `previewUrl` is not used as the `url` in the JSON-LD, because it is not the product's final address.

**Content source.** The DE/EN/TR copy is based only on a read-only visit of the staging site (stories A1–C2 with sentence translations, word lookup, read-aloud with device voices, vocabulary by level, telc/Goethe-format practice, settings, PWA install, Wiktionary/WikDict credits, version 0.1.0). The technology list shows only what is visible from outside (JavaScript, PWA, Web Speech API, a Vite build, Wiktionary data); the owner should confirm or extend it. No user numbers or results are stated.
